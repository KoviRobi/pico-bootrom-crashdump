...
static usb_hw_t *usb_hw_set = hw_set_alias(usb_hw);
static usb_hw_t *usb_hw_clear = hw_clear_alias(usb_hw);
...
#ifndef USB_ASSUME_ZERO_INIT
#define usb_init_clear_deref(x) memset0(x, sizeof(*(x)))
#else
#define usb_init_clear_deref(x) ((void)0)
#endif
#define usb_common_init(ep) ({assert(ep); usb_init_clear_deref(ep);  ep; })

__rom_function_static_impl(void, _usb_transfer_current_packet_only)(struct usb_endpoint *ep);

const struct usb_transfer_type usb_current_packet_only_transfer_type = {
        .on_packet = __rom_function_ref(_usb_transfer_current_packet_only),
...
static struct usb_endpoint *_endpoints[USB_MAX_ENDPOINTS];
...
static void _usb_dump_eps(void)
{
    ...
    for (int num = 1; num < USB_MAX_ENDPOINTS; num++) {
        for(int b = 0; b < 2; b++)
        {
            ...
            { printf(", FULL"); }
            if (ctrl & USB_BUF_CTRL_LAST_BUF)
            { printf(", LAST"); }
            if (ctrl & USB_BUF_CTRL_BUFF_SEL)
            { printf(", SEL"); }
...
#endif
#ifndef USB_BULK_ONLY_EP1_THRU_16
    ep->buffer_stride = 64;
#endif
...
static void _usb_endpoint_hw_init(struct usb_endpoint *ep, __unused uintptr_t data) {
    ...
    *(uint64_t *) (&usb_dpram->ep_buf_ctrl[ep_num].in) = 0;
#endif
...
#endif
    assert(_device.next_buffer_offset <= USB_DEVICE_DPRAM_MAX);
    if (ep_num) {
#if !defined(GENERAL_SIZE_HACKS) || !defined(USB_BULK_ONLY_EP1_THRU_16)
...
#ifdef USB_ASSUME_ENDPOINTS_ARRAY_FULL
    for(uint i = 1; i < count_of(_endpoints); i++) {
        assert(_endpoints[i]);
    }
#endif
    for (uint i = 1; i < count_of(_endpoints); i++) {
#ifndef USB_ASSUME_ENDPOINTS_ARRAY_FULL
        if (_endpoints[i]) {
...
#ifndef USB_MUST_HAVE_DEVICE_ON_CONFIGURE
...
#else
static_assert(offsetof(struct usb_endpoint, current_transfer) == 8, "");
static_assert(offsetof(struct usb_transfer, outstanding_packet) == 16, "");
static_assert(offsetof(struct usb_transfer, type) == 0, "");
static_assert(offsetof(struct usb_transfer_type, on_packet) == 0, "");

static void __attribute__((naked)) __noinline _usb_call_on_packet(__unused struct usb_endpoint *ep) {
    __asm volatile (
    "ldr r1, =_rom_functions\n"
    "ldr r3, [r0, #8]\n"
    "mov r2, #1\n"
    "strb r2, [r3, #16]\n"
    "ldr r3, [r3, #0]\n"
...
static void _usb_handle_transfer(uint ep_num, bool in, uint which) {
    ...
    assert(ep_num < USB_MAX_ENDPOINTS);
    if (ep_num) {
...
#ifndef GENERAL_SIZE_HACKS
    memset0(transfer, sizeof(struct usb_transfer));
#else
...
#ifndef USB_NO_INTERFACE_ALTERNATES
...
static struct usb_interface *_usb_find_interface(uint num) {
    ...
    if (config) {
#ifdef USB_ZERO_BASED_INTERFACES
...
static void _usb_handle_setup_packet(struct usb_setup_packet *setup) {
    ...
    switch (setup->bmRequestType & USB_REQ_TYPE_RECIPIENT_MASK) {
...
#ifndef USB_NO_DEVICE_SETUP_HANDLER
        ...
        case USB_REQ_TYPE_RECIPIENT_ENDPOINT: {
            ...
            if (endpoint) {
#ifndef USB_NO_ENDPOINT_SETUP_HANDLER
...
struct usb_interface *usb_interface_init(struct usb_interface *interface, const struct usb_interface_descriptor *desc,
...
#ifndef USB_NO_INTERFACE_ENDPOINTS_MEMBER
    ...
    for (uint i = 0; i < endpoint_count; i++) {
        ...
        assert(ep_num && ep_num < USB_MAX_ENDPOINTS);
        ...
        if (USB_TRANSFER_TYPE_ISOCHRONOUS == (ep_desc->bmAttributes & USB_TRANSFER_TYPE_BITS)) {
            endpoints[i]->buffer_stride = 128 << USB_ISOCHRONOUS_BUFFER_STRIDE_TYPE;
        } else {
            endpoints[i]->buffer_stride = 64;
        }
        assert(ep_desc->wMaxPacketSize <= endpoints[i]->buffer_stride);
#endif
#ifdef USB_NO_INTERFACE_ENDPOINTS_MEMBER
        assert(ep_num && ep_num < count_of(_endpoints));
        _endpoints[ep_num] = endpoints[i];
#endif
    }
    return interface;
}
...
void usb_device_start() {
    ...
    interrupt_enable(USBCTRL_IRQ, 1);
}
