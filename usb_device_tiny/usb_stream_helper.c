...
__rom_function_static_impl(void, _usb_stream_packet_packet_handler)(struct usb_endpoint *ep) {
#ifdef USE_BOOTROM_GPIO
...
//        }
        ...
        if (!chunk_offset) {
            memset0(transfer->chunk_buffer, transfer->chunk_size);
        }
...
static const struct usb_transfer_type _usb_stream_transfer_type = {
        .on_packet = __rom_function_ref(_usb_stream_packet_packet_handler)
};
...
void usb_stream_noop_on_packet_complete(__removed_for_space(__unused struct usb_stream_transfer *transfer)) {}
bool usb_stream_noop_on_chunk(uint32_t size __comma_removed_for_space(__unused struct usb_stream_transfer *transfer)) {
    return false;
}
#endif
